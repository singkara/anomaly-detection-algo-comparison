...
def _cae_ocsvm_experiment(dataset_load_fn, dataset_name, single_class_ind, gpu_q):
    ...
    pg = ParameterGrid({'nu': np.linspace(0.1, 0.1, num=1),
                        'gamma': np.logspace(-5, -5, num=1, base=2)})
...
def run_experiments(load_dataset_fn, dataset_name, q, n_classes):
    ...
    # CAE OC-SVM
    processes = [Process(target=_cae_ocsvm_experiment, args=(load_dataset_fn, dataset_name, c, q)) for c in range(n_classes)]
    for p in processes:
        p.start()
        p.join()
